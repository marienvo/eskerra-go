...
> References to future work on the former `podcasts` slice are historical and no longer actionable.

Last reviewed: 2026-08-08 (**Q1 complete and deleted** — the end-of-series ArchUnit rule landed enforced: `viewModelsLiveInTheirFeatureSliceNotInApp` in `ArchitectureLayerRulesTest.kt`, with ADR-001's Consequences note updated from "pending" to enforced. The Q1 phase text is gone from `make-slices-real.md`; Q2, Q3, Q4 and Q5 are unblocked, Q2 or the `workspace-setup.md` disposal being the smallest next steps. Prior review 2026-08-08: batch 6 landed `AppSyncViewModel` + `BinariesViewModel` → `feature/sync/`, leaving `app/` with `AppGateViewModel` only. Prior review 2026-08-08: Q1 batch 5 landed — `SyncSettingsViewModel` + `VaultSettingsViewModel` → `feature/sync/` with its slice README and the "no `feature/settings/` yet" decision. Prior review 2026-08-08: Q1 batch 4 landed — `WorkspaceSetupViewModel` → `feature/setup/` with its slice README. Prior review 2026-08-08: Q1 batch 3 landed — `TodayHubViewModel` → `feature/todayhub/` with its slice README, and the batch-3-triggered ADR-001 amendment. Prior review 2026-08-08: Q1 batch 2 landed — `NoteEditorViewModel` → `feature/editor/`, `NoteReaderViewModel` → `feature/note/`, both slice READMEs; the "frozen-store rekey" claim corrected to "store grows on move" and Q5.2 re-sequenced after Q1 as a consequence. Prior review 2026-08-02: stack rewritten from the 2026-07-05 audit + notebox 2026-07-19 sibling bar; `make-slices-real.md` added; parity plan re-verified; `workspace-setup.md` marked for disposal; the always-on-sync plan executed in full, absorbed into the architecture docs, and deleted, closing parity P1b). Re-review whenever a plan completes or a new one lands.
...
| **Status** | **Active now** (Q0 and **Q1 done and deleted** — moves 10/10 plus the ArchUnit placement rule enforced; Q2–Q5 open) | **Active** (P0 whenever a device session is available; P1a after P0; P1b done) |
| **Deletion condition** | README counter full (8/8), both ArchUnit rules enforced (VM placement done; core→data pending Q3), Q5 residuals merged, **and Q4 resolved** — done and its routing/shell decision absorbed into ADR-001 or `app-contract.md`, or explicitly parked/transferred to a named surviving doc; Q4 skipped does not count (its §Deletion) | P0, P1a, and P2 done (P1b done), `app-contract.md` updated, m4b gate re-pointed (its §5) |
...
4. ~~**Quality Q1 — VM moves + placement guardrail**~~ *(done 2026-08-08 — 10/10 moved, ArchUnit rule enforced; phase deleted).* **Quality Q2** backfills READMEs for the slices no move covers (`podcasts`, `menu`) as its own small doc PR. Q6 test splits (G4) may interleave anytime, respecting move-first-then-split per file.
...
6. **Quality Q3–Q5 — inversion, shell thinning, guardrail residuals** (all now unblocked: the Q1 moves they waited on are done; Q5 opportunistic).
...
- **Next PR (agent-drivable, small):** **Quality Q2** — slice READMEs for `podcasts` and `menu` (G2, its own doc PR), which fills the last counter and is the cheapest remaining quality step. Equally available: the `workspace-setup.md` disposal, or **Quality Q3 step 1** (`AppGateResolver` / `AppGateState`) if a code PR is preferred over docs.
- **Quality Q1 is closed** (2026-08-08, seven PRs): `SearchViewModel` (Q0) → `feature/search/`; `InboxViewModel` → `feature/inbox/`; `NoteEditorViewModel` → `feature/editor/`, `NoteReaderViewModel` → `feature/note/`; `TodayHubViewModel` → `feature/todayhub/`; `WorkspaceSetupViewModel` → `feature/setup/`; `SyncSettingsViewModel` + `VaultSettingsViewModel`, then `AppSyncViewModel` + `BinariesViewModel` → `feature/sync/`; then the ArchUnit placement rule. `app/` holds `AppGateViewModel` only, and now provably so.
...
- **Do not touch yet:** parity P1a (blocked on P0); m4b Go work (blocked on P1a); scheduled background sync (Tier D in `app-contract.md` — a product decision, not backlog); proactive splitting of the pinned podcasts files (split-before-grow only, on touch).
