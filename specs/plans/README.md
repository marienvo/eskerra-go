...
Last reviewed: 2026-08-08 (Q1 batch 5 landed — `SyncSettingsViewModel` + `VaultSettingsViewModel` → `feature/sync/` with its slice README, which also records the "no `feature/settings/` yet" decision; next action bumped to batch 6, the last of the series. Prior review 2026-08-08: Q1 batch 4 landed — `WorkspaceSetupViewModel` → `feature/setup/` with its slice README. Prior review 2026-08-08: Q1 batch 3 landed — `TodayHubViewModel` → `feature/todayhub/` with its slice README, and the batch-3-triggered ADR-001 amendment. Prior review 2026-08-08: Q1 batch 2 landed — `NoteEditorViewModel` → `feature/editor/`, `NoteReaderViewModel` → `feature/note/`, both slice READMEs; the "frozen-store rekey" claim corrected to "store grows on move" and Q5.2 re-sequenced after Q1 as a consequence. Prior review 2026-08-02: stack rewritten from the 2026-07-05 audit + notebox 2026-07-19 sibling bar; `make-slices-real.md` added; parity plan re-verified; `workspace-setup.md` marked for disposal; the always-on-sync plan executed in full, absorbed into the architecture docs, and deleted, closing parity P1b). Re-review whenever a plan completes or a new one lands.
...
| **Use it for** | VM moves with their slice README as mandated companion (Q0–Q1), README backfill for the slices no move covers (Q2), dependency inversion (Q3), app-shell thinning (Q4), CI zone gate / ArchUnit ratchet / CODEOWNERS (Q5), test splits (Q6) | P0 verified inventory, P1a non-inbox plain-Markdown editing, P2 contract adoption + small items (P1b is closed — delivered by the always-on-sync work, plan retired) |
...
| **Status** | **Active now** (Q0 done; Q1 batches 1–5 done; batch 6 is the next PR and the last of the series) | **Active** (P0 whenever a device session is available; P1a after P0; P1b done) |
| **Deletion condition** | Counters full (10/10 + 8/8), ArchUnit rules enforced, Q5 residuals merged, **and Q4 resolved** — done and its routing/shell decision absorbed into ADR-001 or `app-contract.md`, or explicitly parked/transferred to a named surviving doc; Q4 skipped does not count (its §Deletion) | P0, P1a, and P2 done (P1b done), `app-contract.md` updated, m4b gate re-pointed (its §5) |
...
The two tracks interleave freely **except** where a step below names a hold. Quality PRs are small and G1-heavy (agent-drivable); parity P0/P1a need product judgment and a device — pull from whichever track matches the session, taking the lowest open step in that track.
...
4. **Quality Q1 — batch VM moves** *(unblocked; the default next PR — batches 1–5 landed, finish with batch 6, `AppSyncViewModel` + `BinariesViewModel` → `feature/sync/`)*, each PR carrying its destination slice's README as mandated companion documentation. Per the Q0 retro: two VMs per PR when slice-coherent (one for `todayhub`/`sync`), and the `app/archunit_store/` growth rides in the G1 commit. **Quality Q2** then backfills READMEs for the slices no move covers (`podcasts`, `menu`) as its own small doc PR — it is not executed inside a Q1 PR. Q6 test splits (G4) may interleave anytime, respecting move-first-then-split per file.
5. **Parity P1a — non-inbox plain-Markdown editing** (after P0; its own disposable work doc and its own PR). P1a is the contract change: `app-contract.md` + AGENTS.md flip in the same PR as the domain rule. **Hold:** no Q-track PR touching the same slices/use-cases in the same review window; Q3's G3 inversions touch the same sync use cases — whichever runs second waits.
...
7. **m4b audiobook Go phase** — gated on **P1a** completing; the former P1b half of this gate is satisfied (the always-on-sync work landed 2026-08-02). Gate lives in the notebox m4b plan §15, Phase 3, and here. This is a portfolio-sequencing / review-attention gate, not a technical dependency. Never interleaves with Q-track moves.
...
- **Next PR:** quality **Q1 batch 6** — `AppSyncViewModel` + `BinariesViewModel` → `feature/sync/` (G1, including the `app/archunit_store/` update) + an **update** to the existing `feature/sync/README.md` key-files and state-owner sections (G2). `App.kt` is yellow-tier in that work order (sync orchestration wiring), and `app/src/test/.../FakeRemoteSyncRepository.kt` — shared with `ReconcileWorkspaceConfigTest` — must be resolved deliberately rather than dragged along. This closes Q1 and unblocks the end-of-series ArchUnit placement rule, Q3 and Q4. Checks: `./scripts/check-module-budgets.sh` && `./scripts/gradle.sh :app:ktlintCheck :app:lintDebug :app:testDebugUnitTest`. Batches 1–5 are done (`InboxViewModel` → `feature/inbox/`; `NoteEditorViewModel` → `feature/editor/`, `NoteReaderViewModel` → `feature/note/`; `TodayHubViewModel` → `feature/todayhub/`; `WorkspaceSetupViewModel` → `feature/setup/`; `SyncSettingsViewModel` + `VaultSettingsViewModel` → `feature/sync/`). The **ADR-001 amendment** follow-up is done (landed with batch 3); it must be revisited when the Q1 end-of-series ArchUnit placement rule merges.
- **Batch order yields to in-flight PRs on the same files** — a *warning*, not a hard block: when the next batch's payload overlaps an open PR, either wait for that PR to merge and branch off the new tip, or take the next non-overlapping batch. Never rebase a move on top of an unmerged behavior change on the same file.
...
- **Do not touch yet:** parity P1a (blocked on P0); Q3's G3 inversions (after Q1 batches 1–4); CODEOWNERS (after Q1); m4b Go work (blocked on P1a); scheduled background sync (Tier D in `app-contract.md` — a product decision, not backlog); proactive splitting of the pinned podcasts files (split-before-grow only, on touch).
