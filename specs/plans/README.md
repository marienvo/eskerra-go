...
| **Use it for** | VM moves with their slice README as mandated companion (Q0–Q1), README backfill for the slices no move covers (Q2), dependency inversion (Q3), app-shell thinning (Q4), CI zone gate / ArchUnit ratchet / CODEOWNERS (Q5), test splits (Q6) | P0 verified inventory, P1a non-inbox plain-Markdown editing, P2 contract adoption + small items (P1b was delivered by `always-on-sync-and-spinner.md` and is closed) |
...
| **Status** | **Active now** (Q0 done; Q1 batch 1 is the next PR) | **Active** (P0 whenever a device session is available; P1a after P0; P1b done via `always-on-sync-and-spinner.md`) |
...
|---|---|---|
| `always-on-sync-and-spinner.md` | **Disposal pending** (product track; Phases A–C done) | Sync spinner, write-triggered auto-sync, and boot/foreground sync all landed. **Delivered parity P1b, now closed.** Remaining: Phase D — absorb the durable rules into `app-contract.md` + `sync-hardening-and-recovery.md`, then delete this plan and its rows here. |
...
5. ~~**`always-on-sync-and-spinner.md` Phases A–C**~~ *(done on `always-on-sync-spinner`; gate green)* — spinner, write-triggered sync, boot/foreground sync. These **delivered parity P1b**, which is now closed. Only **Phase D** (absorb + delete the plan) remains, as its own small doc PR.
6. **Parity P1a — non-inbox plain-Markdown editing** (after P0; its own disposable work doc and its own PR). P1a is the contract change: `app-contract.md` + AGENTS.md flip in the same PR as the domain rule. **Hold:** no Q-track PR touching the same slices/use-cases in the same review window; Q3's G3 inversions touch the same sync use cases — whichever runs second waits.
...
8. **m4b audiobook Go phase** — gated on **P1a** completing; the former P1b half of this gate is satisfied (`always-on-sync-and-spinner.md` Phases A–C landed). Gate lives in the notebox m4b plan §15, Phase 3, and here. This is a portfolio-sequencing / review-attention gate, not a technical dependency. Never interleaves with Q-track moves.
...
- **Independent small doc PRs (each gets its own PR):** `always-on-sync-and-spinner.md` Phase D (absorb + delete); `workspace-setup.md` disposal; the AGENTS.md broken-link fix (`android-vault-notes-rebuild-plan.md` — see `make-slices-real.md` §Follow-ups; it may join another PR only when that PR already edits the same documentation surface).
