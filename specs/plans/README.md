...
Last reviewed: 2026-08-08 (Q1 batch 3 landed — `TodayHubViewModel` → `feature/todayhub/` with its slice README, and the batch-3-triggered ADR-001 amendment; next action bumped to batch 4. Prior review 2026-08-08: Q1 batch 2 landed — `NoteEditorViewModel` → `feature/editor/`, `NoteReaderViewModel` → `feature/note/`, both slice READMEs; the "frozen-store rekey" claim corrected to "store grows on move" and Q5.2 re-sequenced after Q1 as a consequence. Prior review 2026-08-02: stack rewritten from the 2026-07-05 audit + notebox 2026-07-19 sibling bar; `make-slices-real.md` added; parity plan re-verified; `workspace-setup.md` marked for disposal; the always-on-sync plan executed in full, absorbed into the architecture docs, and deleted, closing parity P1b). Re-review whenever a plan completes or a new one lands.
...
| **Status** | **Active now** (Q0 done; Q1 batches 1–3 done; batch 4 is the next PR) | **Active** (P0 whenever a device session is available; P1a after P0; P1b done) |
...
4. **Quality Q1 — batch VM moves** *(unblocked; the default next PR — batches 1–3 landed, continue with batch 4, `WorkspaceSetupViewModel` → `feature/setup/`)*, each PR carrying its destination slice's README as mandated companion documentation. Per the Q0 retro: two VMs per PR when slice-coherent (one for `todayhub`/`sync`), and the `app/archunit_store/` growth rides in the G1 commit. **Quality Q2** then backfills READMEs for the slices no move covers (`podcasts`, `menu`) as its own small doc PR — it is not executed inside a Q1 PR. Q6 test splits (G4) may interleave anytime, respecting move-first-then-split per file.
...
- **Next PR:** quality **Q1 batch 4** — `WorkspaceSetupViewModel` → `feature/setup/` (G1, including the `app/archunit_store/` update) + the `setup` slice README (G2). Checks: `./scripts/check-module-budgets.sh` && `./scripts/gradle.sh :app:ktlintCheck :app:lintDebug :app:testDebugUnitTest`. Batches 1–3 are done (`InboxViewModel` → `feature/inbox/`; `NoteEditorViewModel` → `feature/editor/`, `NoteReaderViewModel` → `feature/note/`; `TodayHubViewModel` → `feature/todayhub/`). The **ADR-001 amendment** follow-up is done (landed with batch 3); it must be revisited when the Q1 end-of-series ArchUnit placement rule merges.
