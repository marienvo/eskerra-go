...
| **Status** | **Active now** (Q0 done; **Q1 moves done 10/10** — only its ArchUnit guardrail is left, and it is the next PR) | **Active** (P0 whenever a device session is available; P1a after P0; P1b done) |
...
4. **Quality Q1 — VM moves** *(moves done 10/10; only the end-of-series ArchUnit rule remains — G5, maintainer, its own PR)*. **Quality Q2** backfills READMEs for the slices no move covers (`podcasts`, `menu`) as its own small doc PR — it is not executed inside a Q1 PR. Q6 test splits (G4) may interleave anytime, respecting move-first-then-split per file.
...
- **Next PR:** the **Q1 end-of-series ArchUnit rule** (G5, maintainer, small): "no class named `*ViewModel` resides in `..app..` except `AppGateViewModel`", so the hub cannot re-form now that all 10 moves have landed. Its PR also revisits the dated ADR-001 Consequences note (which still says the rule is pending) and then deletes `make-slices-real.md` §Q1. Checks: `./scripts/check-module-budgets.sh` && `./scripts/gradle.sh :app:ktlintCheck :app:lintDebug :app:testDebugUnitTest`.
- **The Q1 move series is complete** (2026-08-08, six PRs): `SearchViewModel` (Q0) → `feature/search/`; `InboxViewModel` → `feature/inbox/`; `NoteEditorViewModel` → `feature/editor/`, `NoteReaderViewModel` → `feature/note/`; `TodayHubViewModel` → `feature/todayhub/`; `WorkspaceSetupViewModel` → `feature/setup/`; `SyncSettingsViewModel` + `VaultSettingsViewModel`, then `AppSyncViewModel` + `BinariesViewModel` → `feature/sync/`. `app/` holds `AppGateViewModel` only.
