...
Delay: `runAfterInteractions` + fixed **6000 ms** delay (`backgroundGeneralReconcileDelayMs`; **0** under Jest).
...
#### 7.3.1 Kotlin merge algorithm (`PodcastsMdMerge.kt`)

Authoritative implementation: `apps/mobile/android/app/src/main/java/com/eskerra/podcast/rss/PodcastsMdMerge.kt`. Tests: `PodcastsMdMergeTest.kt`.

**📻 file refresh (`PodcastRssFileSync.kt`):**

- Parse frontmatter: `rssFeedUrl` (scalar or YAML list), `daysAgo` (default **7**), `timeoutMs` (default **8000**).
- **`minFetchIntervalMinutes`:** present in TypeScript [`podcastRssSync.ts`](../../packages/eskerra-core/src/podcastRssSync.ts) but **not** parsed by Kotlin `FrontmatterRss.kt` — Android native sync ignores this field (intentional parity gap).
- Fetch each feed URL; if **every** fetch fails or no items parse → return file **unchanged** (no `rssFetchedAt` bump).
- Dedupe RSS items by lowercase MP3 URL (keep newest `pubInstant`).
- Rebuild body via `RssMarkdownComposer`; omit items older than `daysAgo` (local calendar cutoff).
- Only frontmatter field updated on success: `rssFetchedAt` (ISO-8601 UTC with `Z`).

**Stub merge into `*- podcasts.md`:**

1. Preserve non-episode prefix lines (everything before first episode bullet).
2. Parse existing episode lines; apply **retention rules** (local calendar, `today` / `yesterday` / 7-day window):
   - **Older than 7 days:** drop.
   - **Between 7 days ago and yesterday (exclusive):** keep only if **not played** (`[ ]`).
   - **Yesterday or today:** keep regardless of played state.
3. From refreshed `📻` bodies: add candidates only for **`today`** and **`yesterday`** air dates; new rows are always unplayed.
4. **Dedupe key:** `{calendarDate}|{normalizedTitle}` where normalized title = lowercase ASCII letters and digits only (`PodcastMarkdownLinks.normalizeTitleKey`). Empty normalized title falls back to `_empty:{hashCode}`.
5. **Collision on same key:** prefer MP3 URL without literal `&amp;` entity; merge `played` with OR; prefer existing article URL when keeping existing MP3.
6. Sort output: descending date, then ascending lowercase MP3 URL.
7. URL sanitizer: `&amp;` → `&` in link destinations (`PodcastMarkdownLinks.kt`).
...
- **Track metadata artwork:** `playEpisode` passes the same resolved URI as the UI (`getCachedPodcastArtworkUri`): `file://` or `https://` from the artwork cache. Never pass raw vault `content://` tree URIs to TrackPlayer (lock-screen / notification artwork uses the same rule as §12.3).
...
- On remote playlist **delete / 404** after content was previously seen (`onRemotePlaylistCleared`): same callback as ETag change → `playlistSyncGeneration` bump → `usePlayer` remote sync effect.
- **Mobile does not filter own-device echoes:** unlike desktop (`useDesktopPlaylistR2EtagPolling` + `isPlaylistR2PollEchoFromOwnDevice`), mobile calls `notifyPlaylistSyncAfterVaultRefresh` for every poll result. A Kotlin rebuild of the Android app must match mobile, not desktop.
...
- Displaying `articleUrl` (`[🌐](url)` prefix on episode lines) in the Episodes list or mini player — parsed and preserved on disk, not shown in UI.
