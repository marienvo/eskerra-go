...
| Sync moments | startup/close/auto + polling | button + every note write + boot + foreground return | **C — in parity** | **Verified** (shipped 2026-08-02; unit-tested triggers) | **P1b done**; only polling stays absent — scheduled background sync remains **Tier D** (app-contract: no WorkManager) |
...
- **P1b — Foreground-resume sync trigger or prompt. → DONE 2026-08-02** (shipped by the always-on-sync work, whose plan is retired), and it went further than the original scope: sync now fires on every note write, on boot, and on every foreground return, with a spinning shell indicator and silent `"!"` failures. Nothing remains to schedule here; the durable behavior lives in [`app-contract.md`](../architecture/app-contract.md) §"Git write and sync channels" + §"Shell sync indicator" and [`sync-hardening-and-recovery.md`](../architecture/sync-hardening-and-recovery.md) §"Automatic vault sync triggers". Scheduled background sync stays **Tier D** (app-contract: no WorkManager). The old hold against `make-slices-real.md` Q3's G3 sync inversions is released — Q3 no longer waits on a parity phase, though it still lands after Q1 batches 1–4.
