...
Status: **planning only** (P1b done 2026-08-02 via `always-on-sync-and-spinner.md`; everything else still planning). Sequenced **before** the M4B audiobook player: `notebox/specs/plans/m4b-audiobook-player.md` Phase 3 (Go player implementation) is gated on **P1a** completing — a portfolio-sequencing / review-attention gate, not a technical dependency (see §4). The audiobook contract/fixture work (its Phases 0–1) is independent and may run in parallel.
...
| Sync moments | startup/close/auto + polling | button + every note write + boot + foreground return | **C — in parity** | **Verified** (`always-on-sync-and-spinner.md` Phases A–C; unit-tested triggers) | **P1b done**; only polling stays absent — scheduled background sync remains **Tier D** (app-contract: no WorkManager) |
...
| Audiobook player | planned (m4b plan) | planned | **C for contract, P for UI** | **Verified** planned-only both sides | after P1a (m4b plan Phase 3; P1b already done) |
...
- **P1b — Foreground-resume sync trigger or prompt. → DONE 2026-08-02, delivered by [`always-on-sync-and-spinner.md`](always-on-sync-and-spinner.md)** (its Phases A–C), which went further than the original scope: sync now fires on every note write, on boot, and on every foreground return, with a spinning shell indicator and silent `"!"` failures. Nothing remains to schedule here; the durable behavior lives in `app-contract.md` §"Git write and sync channels" and `sync-hardening-and-recovery.md` §"Automatic vault sync triggers". Scheduled background sync stays **Tier D** (app-contract: no WorkManager). The old hold against `make-slices-real.md` Q3's G3 sync inversions is released — Q3 no longer waits on a parity phase, though it still lands after Q1 batches 1–4.
- **M4B gate:** m4b Phase 3 (Go player implementation) waits for **P1a** (P1b is done). This is a portfolio-sequencing and review-attention gate — audiobook playback has no technical dependency on note editing or sync moments; the point is that the headline parity gaps are not left half-finished while a new feature competes for the same review attention.
...
Delete when P0, P1a, and P2 are done (P1b already is), `app-contract.md` reflects the new capability + divergence lists, and the m4b plan's Phase 3 gate references app-contract instead of this file. P3 leftovers become backlog notes, not a reason to keep the plan.
